...
export class TokenManager extends MongooseModelManager<IToken> {
    ...
    public findManyByCollection(collection: ICollection, tokenIds: string[]) {
        return super.findMany({ collectionName: collection.name, tokenId: { $in: tokenIds } });
    }

    public updateMany(tokens: any[], properties: string[]) {
        let writes: any[] = [];
    ...
    public deleteByCollection(collection: ICollection) {
        return super.deleteMany({ collectionName: collection.name });
    }

    public deleteManyByTokenId(collection: ICollection, tokenId: string) {
    ...
    public async batchListings(collection: ICollection, from: number, size: number) {
        return await this.model.find({ collectionName: collection.name, "listings.0": { $exists: true } }).skip(from).limit(size);
    }

    public async batchOffers(collection: ICollection, from: number, size: number) {
        return await this.model.find({ collectionName: collection.name, "offers.0": { $exists: true } }).skip(from).limit(size);
    }
}